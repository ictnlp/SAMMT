...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def __init__(self, args, encoder, decoder, ffn_net, image_encoder):
        super().__init__(encoder, decoder, ffn_net, image_encoder)
        self.args = args
    ...
    def build_model(cls, args, task):
        ...
        ffn_net = cls.build_ffn_net(args)
        image_encoder = cls.build_image_encoder(args)
        return cls(args, encoder, decoder, ffn_net, image_encoder)
    ...
    @classmethod
    def build_ffn_net(cls, args):
        return FeedForwardNet(args)

    @classmethod
    def build_image_encoder(cls, args):
    ...
    def forward(
        ...
        """
        encoder_out_syn = self.encoder(
            src_tokens, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens,
            synth_masks_list=synth_masks_list, synth_imgs_list=synth_imgs_list,
            authe_masks_list=authe_masks_list, authe_imgs_list=authe_imgs_list, ffn_net=ffn_net,
            image_encoder=image_encoder
        )
        encoder_out_aut = self.encoder(
            src_tokens, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens,
            synth_masks_list=authe_masks_list, synth_imgs_list=authe_imgs_list,
            authe_masks_list=synth_masks_list, authe_imgs_list=synth_imgs_list, ffn_net=ffn_net,
            image_encoder=image_encoder
        )
        decoder_out = {}
        decoder_out['syn'] = self.decoder(
            prev_output_tokens,
            encoder_out=encoder_out_syn,
        ...
        decoder_out['aut'] = self.decoder(
            prev_output_tokens,
            encoder_out=encoder_out_aut,
            features_only=features_only,
            alignment_layer=alignment_layer,
            alignment_heads=alignment_heads,
            src_lengths=src_lengths,
    ...
    def get_normalized_probs(
        ...
        # print("net_output"+str(len(net_output)))
        return self.get_normalized_probs_scriptable(net_output, log_probs, sample)

    def get_img_feature(
            self,
    ):
        return self.args.encoder_out.img_feature
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.image_dropout_module = FairseqDropout(
            args.image_dropout, module_name=self.__class__.__name__
        )
    ...
    def fuse_img_feat(self, fuse_img_feat, side_img_feat, image_encoder):
    ...
    def forward(
        ...
        idx = 0
        img_features_orig = []
        for img in synth_imgs_list:
            img_features_orig.append(img)
            idx += 1
        img_features_orig = torch.stack(img_features_orig, 0)
        img_features_orig = img_features_orig.view(img_features_orig.size(2), img_features_orig.size(1),img_features_orig.size(3))
        img_features_orig = ffn_net(img_features_orig)
        idx = 0
        for fuse_img, side_img in zip(synth_imgs_list, authe_imgs_list):
            fuse_img = img_features_orig
            img_features.append(
                self.fuse_img_feat(fuse_img, side_img, image_encoder=image_encoder))
            idx += 1
        img_features = torch.stack(img_features, 0)
        img_features = img_features.view(img_features.size(1), img_features.size(2), img_features.size(3))
        txt_len = x.size(0)  # src len (with padding)
        x = torch.cat((x, img_features), 0)  # (T+P) x B x C
        ...
        for layer in self.layers:
            x = layer(x, encoder_padding_mask, txt_len)
            if return_all_hiddens:
                assert encoder_states is not None
                encoder_states.append(x)

        if encoder_padding_mask is not None:
            img_padding_mask = encoder_padding_mask.new(encoder_padding_mask.size(0), img_features.size(0)).fill_(False)
            encoder_padding_mask = torch.cat((encoder_padding_mask, img_padding_mask), 1)
    ...
    def reorder_encoder_out(self, encoder_out: EncoderOut, new_order):
        ...
        new_img_feature = (
            encoder_out.img_feature
            if encoder_out.img_feature is None
            else encoder_out.img_feature.index_select(1, new_order)
        )
        ...
        return EncoderOut(
            ...
            img_feature=new_img_feature,  # T x B x C
...
@register_model_architecture("multimodal_transformer", "multimodal_transformer_sammt")
def multimodal_transformer_sammt(args):
